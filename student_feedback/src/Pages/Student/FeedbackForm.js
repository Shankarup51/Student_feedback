...
const FeedbackForm = () => {
    ...
    const { courseId, courseName, facultyName } = location.state || {}; // Data passed via navigation
    ...
    return (
        <div className="p-6  w-full flex flex-col items-center mx-auto bg-blue-200">
            <h1 className="text-2xl font-semibold leading-snug tracking-wider mb-4 border-b-4 w-full border-yellow-200 shadow-md shadow-black/20 text-center pb-4">Feedback Form</h1>
            ...
            <div className="bg-white/10 border border-purple-300 rounded-lg shadow-md backdrop-blur-sm text-white p-5 mb-6 space-y-2">
                <p className="text-lg text-black">
                    <span className="font-semibold text-purple-500">Course ID:</span> {courseId}
                </p>
                <p className="text-lg text-black">
                    <span className="font-semibold text-purple-500">Course Name:</span> {courseName}
                </p>
                {formType !== 'course' && (
                    <p className="text-lg text-black">
                        <span className="font-semibold text-purple-500">Faculty Name:</span> {facultyName}
                    </p>
                )}
            </div>


            <form onSubmit={handleSubmit} className="space-y-6 bg-white/5 p-6 rounded-xl shadow-md backdrop-blur border border-white/10 text-white max-w-3xl mx-auto">
                {questions.map((q, idx) => (
                    <div key={idx} className="border-b border-white/10 pb-4 space-y-2">
                        ...
                        <div className="flex gap-4 text-base text-purple-300">
                            {[5, 4, 3, 2, 1].map((val) => (
                                <label key={val} className="flex items-center gap-1 hover:scale-105 transition">
                                    <input
                                        type="radio"
                                        name={`q${idx}`}
                                        value={val}
                ...
                <div>
                    <label className="block text-purple-500 font-medium text-lg mb-2 tracking-wide">
                        Any other comment (max 5 points):
                    </label>
                    <textarea
